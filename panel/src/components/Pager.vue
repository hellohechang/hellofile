<template>
  <div v-show="obj.totalPage > 1" class="pagerBox">
    <button @click="changePage('pre')" v-show="pageNo > 1">上一页</button>
    <button @click="changePage(1)" v-show="obj.arr[0] > 1">1</button>
    <button @click="changePage(obj.arr[0] - 1)" v-show="obj.arr[0] > 2">...</button>
    <button :class="item === pageNo ? 'active' : ''" @click="changePage(item)" v-for="(item, idx) in obj.arr"
      :key="idx">{{ item
      }}</button>
    ...
    <button @click="changePage('next')" v-show="obj.arr[obj.arr.length - 1] < obj.totalPage">下一页</button>
    <span>共{{ total }}条</span>
